...
def bit_8(triple_set):
    ...
    y /= count
    z /= count
    bit_pixel = np.array([myround(x,40),myround(y,40),myround(z,40)])
    return bit_pixel
...
def shrink(picture,pix_across):
    ...
    new_picture = np.empty((pix_down,pix_across,3))
    for i in range(pix_down):
        for j in range(pix_across):
            new_picture[i][j] = avg
    return new_picture
...
if __name__ == '__main__':
    ...
    pixelated = shrink(image_matrix, int(sys.argv[2]))
    if sys.argv[3] and sys.argv[3] == 's':
        print(type(pixelated))
        result = Image.fromarray(pixelated)
        result.save('aa' + sys.argv[1])
