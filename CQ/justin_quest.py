...
#----------------------------------------------------- Game Strings
margin = ' '*22
success     = ' '*45 + '\U0001F538' 
speed_bonus = ' '*45 + u"\u2728"
bunny       = ' '*43 + ' \U0001F430'
mistake     = ' '*43 + ' \U0001F480'
sword       = '\U0001F5E1'
shield      = '\U0001F6E1'
frog        = '\U0001F438'
hand        = '\U0001F590'
...
def level_str():
    return ' '*7 + 'level: '+ yellow + f'{str(round(level)):>2}' + cyan + '\n'

def score_str(scr):
    return '   '*14 + ' +'+ yellow + "{0:.2f}".format(scr) + cyan + '\n'
...
def challenge(limit, hardmode = False, lvl = 5,
    ...
    while total_time < limit:
        try:
            ...
            print('\n'*10 +' '*3+ frog + ' never give up, never surrender! ' + red + '      -10')
