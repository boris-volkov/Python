...
scale = sounds.harajoshi_scale # taken from my sounds package
score = 0 
margin = '      '
...
class Board(object):
            ...
            for i in range(3, width):
                for j in range(min(height-a-1, i)):
                    ...
                    try:           #row   col 
                        ...
                        border = True
                                         # things in the diagonal match bottom left   # not counting zeros
                    if self.board[a+j][i-j] != 0 and self.board[a+j][i-j] == self.board[a+j+1][i-j-1]:
                        counter += 1
                        if counter >= 3:              # or it will stop finding matches, peeks two ahead
                            if border or (self.board[a+j+1][i-j-1] != self.board[a+j+2][i-j-2]):
                                change += 1 #boolean to let print know the board changed
                                score+=round(2**(counter+1))
                                for d in range(counter+1):
                                    to_delete.append((a+j+1-d,i-j-1+d))
                    else:
    ...
    green = colors.rgb(0,150,50)
    white = colors.rgb(255,220,180)
    line_color = colors.rgb(200,255,255)
